...
@section('content')
        ...
        <div class="container-fluid">
            <div class="box">
                ...
                <div class="box-body">
                    ...
                    <div class="col-xs-4">
                        <a href="#" data-toggle="modal" data-target="#modal-create-dept" class="btn btn-success">Add</a>
                    </div>
                    {{-- <div class="row"> --}}
                    <div class="col-sm-12">
                            ...
                            <table class="table table-bordered" id="table-list" name="table-list">
                                ...
                                @else
                                    ...
                                    @foreach ($data->data as $entry)
                                        <tr>
                                            <td>{{ $i++ }}</td>
                                            <td class="pd-name">{{ $entry->id_dept }}</td>
                                            <td class="pd-dept">{{ $entry->nama_dept }}</td>
                                            <td>
                                                <a class="btn btn-xs btn-danger"
                                                    onclick="deleteData({{ $entry->id_dept }})">Hapus</a>
                                                {{-- <a class="btn btn-xs btn-success">Edit</a> --}}
                                                <a href="" class="btnSelect" name="btnSelect" data-toggle="modal"
                                                    data-target="#modal-edit-dept">
    ...
    <div class="modal fade" id="modal-create-dept">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">Tambah DEPT</h4>
                </div>
                <form role="form" method="post" action="{{ URL::to('department') }}">
                    @csrf
    ...
    <div class="modal fade" id="modal-edit-dept">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form role="form" method="post" action="{{ URL::to('department/edit') }}">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-xs-12 col-md-12">
                                ...
                                <div class="form-group">
                                    <label class="col-xs-4 col-md-2 control-label"></i>Nama dept</label>
                                    <div class="col-xs-6 col-md-9 input-group">
                                        <input type="text" class="form-control " name="nama_dept_show"
                                            id="nama_dept_show" placeholder="Input dept" required />
                                            <input type="hidden" class="form-control " name="id_dept_show"
                                            id="id_dept_show" placeholder="Input dept" required />
                                    </div>
                                </div>
    ...
    <script>
        $(".btnSelect").on('click', function() {
            ...
            var col3 = currentRow.find(".pd-dept").html();
            $("#nama_dept_show").val(col3);
            $("#id_dept_show").val(col2);
